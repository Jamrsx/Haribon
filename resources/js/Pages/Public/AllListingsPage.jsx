...
export default function AllListingsPage() {
    ...
    const initialFavoriteIds = props.favorite_ids ?? [];
    ...
    const handleOpenInquiry = (e, property) => {
        e.preventDefault();
        e.stopPropagation();
        if (!auth?.user) {
        ...
        console.log('[AllListings] opening inquiry for', property.id);
                    ...
                    {properties.length === 0 ? (
                        ...
                        <>
                            <div className={viewMode === 'list' ? 'space-y-4' : 'grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4'}>
                                ...
                                <div
                                    key={property.id}
                                    className={`group relative rounded-2xl border border-slate-200 bg-white shadow-sm transition-all hover:shadow-md ${
                                ...
                                >
                                    <Link
                                        ...
                                        className={`relative block overflow-hidden bg-slate-100 ${
                                            viewMode === 'list' ? 'w-48 flex-shrink-0 rounded-l-2xl' : 'aspect-square rounded-t-2xl'
                                        }`}
                                    >
                                    ...
                                    {auth?.user?.id !== property.user?.id && (
                                        <div className="absolute right-2 top-2 z-10">
                                            <FavoriteButton
                                                propertyId={property.id}
                                                initialFavorited={favoriteSet.has(property.id)}
                                                size="sm"
                                    ...
                                    <div className={`flex flex-col p-4 ${viewMode === 'list' ? 'flex-1' : ''}`}>
                                        ...
                                        {auth?.user?.id !== property.user?.id && (
                                            <button
                                                type="button"
                                                onClick={(e) => handleOpenInquiry(e, property)}
                                                className="mt-3 inline-flex items-center justify-center gap-1.5 self-start rounded-lg border border-emerald-600 bg-white px-3 py-1.5 text-xs font-medium text-emerald-700 transition hover:bg-emerald-50"
                                            >
